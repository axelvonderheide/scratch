...
class WaistedShape( HasTraits ):
    ...
    dx is a  coefficient of a quadratic term defining the densiity
    of the mesh in the x direction. 
    dx = 0 means uniform mesh
    dx > 0 makes the mesh finer at the left hand side
    '''
    ly0 = Float(  0.5, enter_set = True , auto_set = False )
...
class SimTensileTestWaisted( IBVModel ):
    ...
    def _get_tloop(self):
        ...
        mats = MATS2D5MicroplaneDamage( E = E_c, nu = 0.25,
                                             n_mp = 10,
...
#        fets =  FETS2D58H( mats_eval  = mats ) 
        ...
        bc_plane_yz_1    = BCSlice( var = 'u', value = u_max, dims = [0], slice = domain[-1,:,:,-1,:,:] )
