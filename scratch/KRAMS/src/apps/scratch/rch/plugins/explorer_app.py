...
class Explorer3D(HasTraits):
    ...
    def _add_source_fired(self):
        from ibvpy.mathkit.geo.geo_ndgrid import GeoNDGrid, GridPoint
        ...
        engine = self.window.get_service('enthought.mayavi.core.engine.Engine')
        engine.add_source( mfn )
    ...
    ########################################
    # Our UI view.
    view = View(Item('add_source'),
                Item('equation', editor=TextEditor(auto_set=False,
                                                   enter_set=True)),
                Item('dimensions'),
                Item('volume'),
