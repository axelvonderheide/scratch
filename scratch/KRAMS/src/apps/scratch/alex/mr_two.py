...
#        return bc_col_link_list
    ...
    def _get_lc_dict( self ):
        return {'lc_g' : self.lc_g_list,
                ...
                'lc_g_own_weigth' : self.lc_g_own_weigth_list,
    ...
    lc_g_own_weigth_list = Property( List, depends_on = '+ps_levels, +input' )
    @cached_property
    def _get_lc_g_own_weigth_list( self ):
        '''loading case: dead loads 
        '''
        #slices   
        #
        roof_0 , roof_1 = self.fe_grid_roofs
    ...
    def _get_lc_g_surf_load_list( self ):
        '''loading case: additional surface loads (usbaulasten)
        '''
        #slices   
        #
        roof_0 , roof_1 = self.fe_grid_roofs
...
#    do = 'cs' 
    ...
    if do == 'ui':
        ...
        # input options
        #
        sim_model.link_type = 'exc_V_ip'
#        sim_model.link_type = 'inc_V_ip'
#        sim_model.lc = 'lc_w_asym'
#        sim_model.lc = 'lc_g'
        sim_model.lc = 'lc_g_own_weigth'
        sim_model.lc = 'lc_g_surf_load'
        sim_model.lc = 'lc_g_tol_sym'
#        sim_model.lc = 'lc_g_tol_asym'
#        sim_model.lc = 'lc_g_edge_load'
#        sim_model.lc = 'lc_s_sym'
#        sim_model.lc = 'lc_w_pos'

        sim_model.n_elems_xy_quarter = 17 # corresponds to 'equal_100cm'
#        sim_model.n_elems_xy_quarter = 10 # needs to be defined before shift array otherwise 
#                                          # linear function not evaluable

        sim_model.select_hinge_pos = True # needs to be true for shift

#        sim_model.shift_array = array( [[2, 2, 1]] ) # shift for one quarter

        # corresponds to 'equal_100cm'
        #
        sim_model.shift_array = array( [[0.45 / 2 ** 0.5, 0.45 / 2 ** 0.5, 2],
                                        [0.5, 0.5 , 1],
                                        [1.5 , 1.5, 4],
                                        [2.5, 2.5, 4],
                                        [3.5, 3.5, 4]] )
