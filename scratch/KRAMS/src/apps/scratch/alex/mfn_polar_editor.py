...
def coord_trans_plt(r_value, frac_noplot, plotrange_min, plotrange_max):
    '''transform the values of the radius function to the corresponding values
    in the polar editor based on the specified plotranges 
    '''
    r_a = frac_noplot   
...
def _get_radius_arr_plt(radius_arr, frac_noplot, plotrange_min, plotrange_max):
    '''return the array with transformed values of radius ready to get plotted
    '''
...
def unitcircle_fn( theta_value, frac_noplot, plotrange_min, plotrange_max):
    '''get the plot radius that corresponds to radius = zero
    '''
...
def _get_unitcircle_arr(theta_arr, frac_noplot, plotrange_min, plotrange_max):
    '''return the array with transformed values of unitcircle ready to get plotted
    '''
...
class MFnPolarPlotEditor ( Editor ):
    ...
    info = Property( Float, depends_on = 'plotrange_min, plotrange_max' )
    @cached_property
    # check for plausibility and return an information or an error message
    def _get_info(self):
        if self.plotrange_min >= self.plotrange_max:
            info = '### Error ###: invalid plot ranges. plotrange_max must be greater than plotrange_min.'
#        elif not ( self.plotrange_min <= self.radius_min and self.radius_max <= self.plotrange_max ):
#            info = '# Note #: Some value of the function are out of the specified plot ranges!'
        else:
    ...
    def update_editor(self):
        ...
        try:
            ...
            plotrange_min = self.plotrange_min
            plotrange_max = self.plotrange_max
            frac_noplot   = self.frac_noplot
...
#            frac_noplot   = getattr(self.object, plotitem.value_list[3])
    ...
    traits_view = View( Item('plotrange_min'),
                          Item('plotrange_max'),
                          Item('frac_noplot'),
                   resizable = True,
                   #scrollable = True,
