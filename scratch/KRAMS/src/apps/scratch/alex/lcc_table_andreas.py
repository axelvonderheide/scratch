...
class LCCTable( HasTraits ):
    ...
    # @todo: consitency check is called at the wrong time of construction caused by filtering.
    #
#    @on_trait_change( 'lc_list' )
    ...
    #
    imposed_idx_list = Property( List )
    @cached_property
    ...
    def _get_combi_arr( self ):
        ...
        #---------------------------------------------------------------
        # get permutations of safety factors 
        #---------------------------------------------------------------
        #
        ...
        #---------------------------------------------------------------
        # get leading and non leading combination factors
        #---------------------------------------------------------------
        ...
        # 
            ...
            for imposed_idx in self.imposed_idx_list:
                ...
                combi_arr_lead_i = combi_arr * psi_arr
            ...
            #
            combi_arr_psi = vstack( combi_arr_psi_list )

        #---------------------------------------------------------------
        # get exclusive loading cases
        #---------------------------------------------------------------
        ...
        #---------------------------------------------------------------
        # create unique array with only unique load case combinations
        #---------------------------------------------------------------
        # check weather the specified exclusive cases have an effect on the 
        # combination array. This is not the case if the psi values
        # of those imposed-loads have already led to zero entries in the
        # combi_arr. This would yield duplicates in the 'combi_arr' which 
        # need to be removed.
        #
        combi_psi_exclusive_unique_list = [ combi_arr_psi_exclusive[0] ]
        for row in combi_arr_psi_exclusive:
            for row_unique in combi_psi_exclusive_unique_list:
                if len( setdiff1d( row, row_unique ) ):
                    combi_psi_exclusive_unique_list.append( row )
        combi_arr_psi_exclusive_unique = vstack( combi_psi_exclusive_unique_list )
...
class LCCTable_SLS( LCCTable ):
    ...
    # and non leading load cases
    #
    psi_lead_arr = Property( Array )
    def _get_psi_lead_arr( self ):
        return self.psi_lead_dict[ self.combination_SLS ]

    psi_non_lead_arr = Property( Array )
    def _get_psi_non_lead_arr( self ):
...
#                     exclusive_to = ['T_pos'], psi_0 = 0.8, psi_1 = 0.7, psi_2 = 0.5 ),
    ...
    lcg = LCCTable_SLS( data_dir = data_dir, lc_list = lc_list, cut_z_fraction = 0.2 )
