...
class FETSStrongWeakTFSOPU(FETSLSEval):

    x_slice = slice(0, 4)

    sopu_fets = Instance(FETSEval)
    ...
    def get_N_mtx(self,r_pnt, node_ls_values, r_ls_value):
        ...
        N_mtx_sopu = self.sopu_fets.get_N_mtx(r_pnt)
#        n_nodes = self.parent_fets.n_e_dofs/self.parent_fets.n_nodal_dofs
        p_value = sign(r_ls_value)
        #strong
        N_e_s_mtx = hstack([N_mtx_sopu[:2,i*4:i*4+2]* \
                    (p_value-sign(node_ls_values[i]))\
                    for i in range(0,4)])#TODO: works just for bilinear now
        #weak
        #N_e_w_mtx = p_N_mtx[2:] * self._get_Psi(node_ls_values, p_N_mtx)[:,None]
        Psi_ip = abs(r_ls_value)
        Psi_I = abs(node_ls_values)
#        N_e_w_mtx = hstack([N_mtx_sopu[2:,i*4:i*4+4]* \
#                    Psi_I[i]\
#                    for i in range(0,4)])-Psi_ip
        PSI = hstack([N_mtx_sopu[3,i*4+3]* \
                        Psi_I[i]\
                        for i in range(0,4)])-Psi_ip
        N_e_w_mtx = hstack([N_mtx_sopu[2:,i*4+2:i*4+4]*\
                            PSI[i]\
                        for i in range(0,4)])           
        
        
        N_e_mtx = zeros((4,self.n_e_dofs/2))
        #print 'left ',N_e_mtx[3,3::4],' right ',N_e_w_mtx[1,3::4]
        N_e_mtx[0,0:16:4] = N_e_s_mtx[0,0::2]
        N_e_mtx[1,1:16:4] = N_e_s_mtx[1,1::2]
        N_e_mtx[2,2:16:4] = N_e_w_mtx[0,0::2]
        N_e_mtx[3,3:16:4] = N_e_w_mtx[1,1::2]
    ...
    def get_dNr_psi_s_mtx(self,r_pnt, node_ls_values, r_ls_value):
        ...
        '''
        dNr_mtx_sopu = self.sopu_fets.get_dNr_mtx(r_pnt)
        p_value = sign(r_ls_value)
        #n_nodes = self.parent_fets.n_e_dofs/self.parent_fets.n_nodal_dofs
        dNr_mtx = hstack( [ dNr_mtx_sopu[:,i:i+1]* \
                           (p_value\
                           - sign(node_ls_values[i])) for i in range(0,4) ])#TODO:works just with bilinear
        return dNr_mtx
...
#        p_dNr_mtx = self.get_dNr_mtx(r_pnt)
        ...
        N_red = vstack((N_mtx_sopu[2,2::4],N_mtx_sopu[3,3::4]))
        
        second = np_sum((abs(node_ls_values)[:4] * dNr_mtx_sopu), axis = -1)
        
        third = np_sum((N_red[0] * abs(node_ls_values)[:4]))
        
        fourth = np_sum((dNr_mtx_sopu * node_ls_values[:4]), axis = -1)
        
        A_mtx = N_red * ( second - sign(r_ls_value)*fourth)[:,None]
        #A_mtx = p_N_red * ( -1.* sign(r_ls_value))
        B_mtx = dNr_mtx_sopu * (third - abs(r_ls_value))
