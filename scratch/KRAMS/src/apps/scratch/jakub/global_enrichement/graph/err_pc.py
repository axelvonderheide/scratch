...
y1 = 1.-np.array([0.973495364189,0.96183681488,0.948169231415,0.926151096821,0.892445206642,0.816784799099,0.67709505558], dtype = float)
p1 = plt.loglog(x1, y1, 'bo-') 

#bicubic- s
x2 = np.array([64,46,34,22,16,10,4], dtype = int)*2 
y2 = 1.-np.array([0.969193577766,0.957041442394,0.94179970026,0.910250484943,0.877590060234,0.810873627663, 0.635487318039], dtype = float)
p2 = plt.loglog(x2, y2, 'rD--') 

#bicubic - l
x3 = np.array([64,46,34,22,16,10,4], dtype = int)*2 
y3 = 1.-np.array([0.978136539459,0.968259990215,0.955925881863,0.929118335247,0.896266877651,0.825833976269, 0.662910699844], dtype = float)
p3 = plt.loglog(x3, y3, 'g^:') 

plt.xlim(7,150)
#plt.ylim(0.60,1.1)
plt.legend( (p1[0], p2[0],p3[0]), (r'bilinear', r'bicubic serendipity', r'bicubic lagrange'), loc='upper right', prop = {'size':20,
                                                                                  'family':'serif'})
