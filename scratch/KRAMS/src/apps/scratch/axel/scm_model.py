...
from apps.scratch.axel.steel_bar_with_short_fibers import SteelBarwithShortFibers
...
class SCMModel(HasTraits):
    ...
    #    return MultifilamentYarn( length = self.length )
        return SteelBarwithShortFibers(length = self.length)
    ...
    length = Float(300., auto_set = False, enter_set = True,
    ...
    def _get_random_field(self):
        ...
        #checks all values of the array for negativism
        if rf.any() <= 0:
...
#        #sums up cbs_responses and uncracked composite strain 
