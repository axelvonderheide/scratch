...
from apps.scratch.axel.Test_Ar import Ar as Ar_alc
...
class ShortFibersMonteCarlo(HasTraits):
    ...
    def sum_of_fiberforces(self, crack_x , le_array , phi_array , f_array  , tau_array):
        ...
        rv = self.r / np.sin(Pi / 2. - phi_matrix) * active_fibers_matrix
        ...
        mask = (active_fibers_array <= 0.0001 * np.ones(len(active_fibers_array)))
        active_fibers_array += mask * 0.000000000000000000001 
        ...
        ###########TESTPLOT#################
        #plt.plot(crack_x, active_fibers_array)
        #z = np.linspace(0, self.fiber_length / 2, 50)
        #arres = (self.fiber_length - 2 * z) ** 2 / self.fiber_length ** 2
        #plt.plot(z, arres * len(phi_array), 'red')
        #plt.show()
        ###################################
        return result_arr
