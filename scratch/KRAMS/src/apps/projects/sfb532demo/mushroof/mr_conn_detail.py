...
from geo_square_to_circle import GeoSquare2Circle
...
class MRDetail( MushRoofModel ):
    ...
    def _get_bc_fix_dofs_in_the_hollow( self ):
        ...
        s_list = [ roof[ idx_min:idx_max - 1, idx_min:idx_max - 1, :, 1:, 1:, : ],
                   roof[ idx_max - 1, idx_min:idx_max - 1, :, 1:-1, :, : ],
                   roof[ idx_min:idx_max - 1, idx_max - 1, :, :, 1:-1, : ],
    ...
    def _get_tloop( self ):
        ...
        bc_list = self.bc_plate_list + self.bc_plate_roof_link_list + \
                  self.bc_roof_deadweight + self.bc_fix_dofs_in_the_hollow

        ts = TS( sdomain = [ roof, plate ],
                 dof_resultants = True,
                 bcond_list = bc_list,
...
if __name__ == '__main__':
    ...
    sim_model = MRDetail( n_elems_z = 1,
                          n_elems_xy_quarter = 4
                           )
