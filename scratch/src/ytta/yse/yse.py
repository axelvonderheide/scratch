...
from enthought.traits.api import HasTraits, Float, Property, cached_property, \
                                Instance, List, on_trait_change, Int, Tuple, Bool, Event
from enthought.traits.ui.api import \
    Spring, View, Item, Tabbed, VGroup, HGroup, VSplit, Group
...
#        return (l/self.l_rho + self.l_rho/(self.l_rho + l))**(-1./self.m_f)
    ...
    def _get_s_r(self):
        ...
        s_r = self.mu_r/(self.m_f**(-1./self.m_f)*c \
                          + self.Nf**(-2./3.) *self.m_f**(-1./self.m_f - 1./3.)*e**(-1./3./self.m_f)*0.996\
                        )
        #print 's_r =',s_r
        return s_r
    ...
    def _get_gamma_r(self):
        c = e**(-1./self.m_f)
        gamma_r = self.s_r*self.m_f**(-1./self.m_f)*sqrt(c*(1-c))/sqrt(self.Nf)
...
#        print 'mu_b =', mu_b
        ...
        # probability of failure for 1 link of length l_b at load level mu_x (21)
        Pf_x = 1. - 0.5**(gl_b/l_f)
#        print 'Pf_x =', Pf_x
        
        # strength standard deviation of mu_b for evaluating the chain of bundles strength (3)
...
#        print 'slb =', s_lb
        ...
        # cumulative strength distribution of 1 bundle l_b at load level mu_x (22)
...
#        print 'CDF_x =', CDF_x
    ...
    def _get_test_length_r(self):
        xdata = array([ self.min_plot_length, self.l_r, self.l_r], dtype = 'float_' )
        ydata = array([ self.mu_r, self.mu_r, self.mu_x*0.0008 ], dtype = 'float_' )
        return ( xdata, ydata )
    
    test_length_f = Property( depends_on = '+modified' )
    @cached_property
    def _get_test_length_f(self):
        xdata = array([ self.min_plot_length, self.l_f, self.l_f], dtype = 'float_' )
        ydata = array([ self.mu_x, self.mu_x, self.mu_x*0.0008 ], dtype = 'float_' )
        return ( xdata, ydata )
