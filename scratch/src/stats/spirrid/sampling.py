...
class FunctionRandomization(HasStrictTraits):
    ...
    def _get_evar_str(self):
        s_list = ['%s = [%g, ..., %g] (%d)' % (name, value[0], value[-1], len(value))
                  for name, value in zip(self.evar_names, self.evar_list)]
    ...
    def _get_tvar_str(self):
        s_list = ['%s = %s' % (name, str(value))
                  for name, value in zip(self.tvar_names, self.tvar_list)]
        return string.join(s_list, '\n')
...
class RandomSampling(HasStrictTraits):
    ...
    def _get_n_rand_vars(self):
        dt = map(type, self.randomization.tvar_list)
        return dt.count(RV)
...
class RegularGrid(RandomSampling):
    ...
    def _get_theta_list(self):
        ...
        '''
        theta_list = []
        for tvar in self.randomization.tvar_list:
    ...
    @cached_property
    def _get_theta(self):
        return orthogonalize(self.theta_list)
    ...
    def get_samples(self, n):
        ...
        otheta = orthogonalize_full(self.theta_list)
...
class TGrid(RegularGrid):
    ...
    def _get_dG_ogrid(self):
        ...
        for i, (tvar, theta) in \
            enumerate(zip(self.randomization.tvar_list, self.theta)):
