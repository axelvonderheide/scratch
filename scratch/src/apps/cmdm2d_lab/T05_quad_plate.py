...
if __name__ == '__main__':
    ...
    dim = '2D5'
    if dim == '3D':
        mats_eval = MATS3DElastic( E = 30025, nu = 0.25)        
        #fets_eval = FETS3D8H(mats_eval = mats_eval ) # [N/mm], [-]
        #fets_eval = FETS3D8H20U(mats_eval = mats_eval ) # [N/mm], [-]
        fets_eval = FETS2D58H20U(mats_eval = mats_eval ) # [N/mm], [-]
    elif dim == '2D5':

#        from promod.simdb import SimDB
#        simdb = SimDB()
#
#        file_name = join( simdb.matdata_dir, '10-a_2D-02-06a.mats' )
...
#        mfn = MFnLineArray( xdata = [0.0, 1.0], ydata = [1.0, 1.0] )
#        print 'ydata', mfn.get_value(0.5)
...
#        
    ...
    #w_max = - 0.001 # [m]
    w_max = - 0.05 # [m]
    total_load = - 4.8 * 0.001 * 0.25 # [MN] 
    
    n_xy = 8
    n_z  = 2

    elem_area = ( L_quater / n_xy )**2
    elem_load = total_load / elem_area 
    ...
    bc_center_load  = BCSlice( var = 'f', value = total_load, dims = [2], slice = domain[-1,-1,-1,-1,-1,-1] )
    bc_center_w     = BCSlice( var = 'u', value = w_max, dims = [2], slice = domain[-1,-1,-1,-1,-1,-1] )
    bc_center_el    = BCSlice( var = 'f', value = elem_load, dims = [2], slice = domain[-1,-1,-1, :, :,-1] )

    bc_center_xline_load  = BCSlice( var = 'u', value = w_max, dims = [2], slice = domain[-2:,-2,-1,:,0,-1] )
    bc_center_yline_load  = BCSlice( var = 'u', value = w_max, dims = [2], slice = domain[-2,-2:,-1,0,:,-1] )
    ...
    ts = TS(
            ...
            bcond_list = [bc_symplane_yz, bc_symplane_xz, bc_support_000, 
                          bc_center_w,
#                          bc_center_xline_load,
#                          bc_center_yline_load
                        ],
             rtrace_list = [      
                         f_w_diagram,
...
#                                        record_on = 'update'),
    ...
    if dim == '3D': 
        tloop = TLoop( tstepper = ts, debug = False,
                       tolerance = 1e-4,
                       tline  = TLine( min = 0.0,  step = 1.0, max = 1.05 ))
    else:
        try:
    ...
    if do == 'eval':
        ...
        #print 'w', f_w_diagram.trace.xdata
        #print 'f', f_w_diagram.trace.ydata
        f_w_diagram.refresh()
...
#        f_w_diagram.trace.mpl_plot( p, color = 'red' )
        ...
        trace = load( file )
        p.plot( trace.xdata  * 1000, trace.ydata, color = 'red' )
