...
from enthought.traits.api import Float, Instance, Array, Int, Property, cached_property, on_trait_change, Bool
...
#    update_e_max_value_new = Bool(False)
#    
#    e_max_value_new = Property(Float, depends_on = 'update_e_max_value_new' )
#    @cached_property
#    def _get_e_max_value_new(self):
#        '''this is a temporary hack to get the current maximum microplane strain
#        run one trial step in order to obtain the maximum
#        microplane strain of the step n+1:
#        @todo: alternatively use 'get_e_max' of the material model using 'eps_n1'
#        '''
#        print '### get e_max_value_new ### '
#
#        mats_state_array_old = copy( self.tloop.tstepper.sctx.mats_state_array )
#        print 'mats_state_array_old ', mats_state_array_old 
#        
#        x = hstack([ self.fitted_phi_fn.xdata[:], self.tmax ])
#        y = hstack([ self.fitted_phi_fn.ydata[:],    1.     ])
#        self.fitted_phi_fn.set( xdata = x, ydata = y )
#        self.fitted_phi_fn.data_changed = True
#        self.run_trial_step()
#        mats_state_array_new = copy( self.tloop.tstepper.sctx.mats_state_array )
#        # get maximum microplane strain:
#        e_max_value = max( mats_state_array_new )
...
#        print '### e_max_value_new = ', 1.001*e_max_value
#        return 1.001*e_max_value
...
#        x = hstack([ self.fitted_phi_fn.xdata[:], self.e_max_value_new ])
        ...
        # ------------------------                
        mats_state_array_old = copy( self.tloop.tstepper.sctx.mats_state_array )
#        print 'state array before trail::::::::::::::::', self.tloop.tstepper.sctx.mats_state_array
                   
        # ------------------------                
        # run trial step: 
        # ------------------------
        print '   current_U_n   =', self.tloop.U_n                
        # try the next equilibrium
        self.run_trial_step()

        # ------------------------                
        # reset mats_state_array to value before trial:
        # ------------------------                
...
#        print 'state array after trail::::::::::::::::', self.tloop.tstepper.sctx.mats_state_array
        self.tloop.tstepper.sctx.mats_state_array[:] = mats_state_array_old[:]
        print '   reset state array'

        # ------------------------                
        # remove the trial value in fitted_phi_fn 
        # ------------------------                
        x = self.fitted_phi_fn.xdata[:-1]
        y = self.fitted_phi_fn.ydata[:-1]
        self.fitted_phi_fn.set( xdata = x, ydata = y )
...
#        print '\n whole sig_app ', self.tloop.rtrace_mngr.rtrace_bound_list[1].trace.ydata, ' \n'
        ...
        print '    lack_of_fit_absolut   ', lack_of_fit
        print '    lack_of_fit_relative  ', lack_of_fit
    ...
    def fit_response(self):
        ...
        for n in range( self.n_steps ):
            ...
            phi_new = phi_old
#            self.update_e_max_value_new = False
            
            try:
                # @todo: check if there should be optional arguments for method
                # 'brentq' such as 'xtol = 1.0e-12 (default)' absolut error
                # or 'maxiter' (so far default values ar used)
                phi_new = brentq( self.get_lack_of_fit, 0., phi_old )#, xtol = 0.01)
...
#                phi_new = brenth( self.get_lack_of_fit, 0., phi_old )
                ...
                print 'No sign change between get_lack_of_fit(phi_old) = ', a 
...
#            print 'state array before update ###:', mats_state_array
    
            # get maximum microplane strain:
            e_max_arr = mats_state_array
            e_max_value = max( e_max_arr )
                        
            # update phi_data:
#            x = hstack([ self.fitted_phi_fn.xdata[:], e_max_value + self.step_size ])
            x = hstack([ self.fitted_phi_fn.xdata[:], current_time + self.step_size  ])
...
#                                                   record_on = 'update' ),
    ...
    fitter.mats2D_eval.polar_fn.n_mp = 50
