...
class InfoShellFileMushroof( HasTraits ):
    ...
    def read_thickness_mushroof_stb( self, file_name ):
        ...
        return  c_[ X, Y, Z, thickness ]

    def read_solicitations_mushroof_stb( self, file_name ):
...
#        column_headings = array(["Nr.","Punkt","X","Y","Z","mx","my","mxy","vx","vy","nx","ny","nxy"])
        ...
        return c_[ elem_no, X, Y, Z, mx, my, mxy, nx, ny, nxy ]
    ...
    # 
    thickness_mushroof_stb_arr = Property( Array, depends_on = 'data_file_thickness' )
    @cached_property
    def _get_thickness_mushroof_stb_arr( self ):
        return self.read_thickness_mushroof_stb( self.data_file_thickness )

    X_ = Property( Array, depends_on = 'data_file_thickness' )
    def _get_X_( self ):
        return self.thickness_mushroof_stb_arr[:, 0]

    Y_ = Property( Array, depends_on = 'data_file_thickness' )
    def _get_Y_( self ):
        return self.thickness_mushroof_stb_arr[:, 1]

    Z_ = Property( Array, depends_on = 'data_file_thickness' )
    def _get_Z_( self ):
        return self.thickness_mushroof_stb_arr[:, 2]

    D_elem = Property( Array, depends_on = 'data_file_thickness' )
    def _get_D_elem( self ):
        # change units from [mm] to [m]:
        return self.thickness_mushroof_stb_arr[:, 3] / 1000.

    # coordinates and solicitations read from file:
    # 
    solicitations_mushroof_stb_arr = Property( Array, depends_on = 'data_file_solicitations' )
    @cached_property
    def _get_solicitations_mushroof_stb_arr( self ):
        return self.read_solicitations_mushroof_stb( self.data_file_solicitations )

    elem_no = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_elem_no( self ):
        return self.solicitations_mushroof_stb_arr[:, 0]

    X = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_X( self ):
        return self.solicitations_mushroof_stb_arr[:, 1]

    Y = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_Y( self ):
        return self.solicitations_mushroof_stb_arr[:, 2]

    Z = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_Z( self ):
        return self.solicitations_mushroof_stb_arr[:, 3]

    mx = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_mx( self ):
        return self.solicitations_mushroof_stb_arr[:, 4]

    my = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_my( self ):
        return self.solicitations_mushroof_stb_arr[:, 5]

    mxy = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_mxy( self ):
        return self.solicitations_mushroof_stb_arr[:, 6]

    nx = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_nx( self ):
        return self.solicitations_mushroof_stb_arr[:, 7]

    ny = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_ny( self ):
        return self.solicitations_mushroof_stb_arr[:, 8]

    nxy = Property( Array, depends_on = 'data_file_solicitations' )
    def _get_nxy( self ):
        return self.solicitations_mushroof_stb_arr[:, 9]
    ...
    def _check_input_files_consistency( self ):
    ...
    def get_outputs_GdG( self, n, m ):
        ...
        eta_tot = eta_n + eta_m
        return c_[ sig_n, sig_m, eta_n, eta_m, eta_tot ]

    # ------------------------------------------------------------
    # T: outputs:
    # ------------------------------------------------------------

    def get_outputs_GdT( self, n, m, alpha ):
...
#            beta_q = abs( alpha )
        ...
        return c_[ e, m_Eds, f_t, beta_l, beta_q, f_Rtex, n_tex ]
    ...
    princ_values_M = Property( Array, depends_on = 'data_file_solicitations' )
    @cached_property
    def _get_princ_values_M( self ):
        ...
        return c_[ m1, m2, alpha_M, mx_M, my_M, nx_M, ny_M ]

    m1 = Property( Float )
    def _get_m1( self ):
        return self.princ_values_M[:, 0]

    m2 = Property( Float )
    def _get_m2( self ):
        return self.princ_values_M[:, 1]

    alpha_M = Property( Float )
    def _get_alpha_M( self ):
        return self.princ_values_M[:, 2]

    mx_M = Property( Float )
    def _get_mx_M( self ):
        return self.princ_values_M[:, 3]

    my_M = Property( Float )
    def _get_my_M( self ):
        return self.princ_values_M[:, 4]

    nx_M = Property( Float )
    def _get_nx_M( self ):
        return self.princ_values_M[:, 5]

    ny_M = Property( Float )
    def _get_ny_M( self ):
        return self.princ_values_M[:, 6]

    # ------------------------------------------------------------
    # Index N: principle normal forces with corresponding moments
    # ------------------------------------------------------------

    princ_values_N = Property( Array, depends_on = 'data_file_solicitations' )
    @cached_property
    def _get_princ_values_N( self ):
        ...
        return c_[ n1, n2, alpha_N, mx_N, my_N, nx_N, ny_N ]

    n1 = Property( Float )
    def _get_n1( self ):
        return self.princ_values_N[:, 0]

    n2 = Property( Float )
    def _get_n2( self ):
        return self.princ_values_N[:, 1]

    alpha_N = Property( Float )
    def _get_alpha_N( self ):
        return self.princ_values_N[:, 2]

    mx_N = Property( Float )
    def _get_mx_N( self ):
        return self.princ_values_N[:, 3]

    my_N = Property( Float )
    def _get_my_N( self ):
        return self.princ_values_N[:, 4]

    nx_N = Property( Float )
    def _get_nx_N( self ):
        return self.princ_values_N[:, 5]

    ny_N = Property( Float )
    def _get_ny_N( self ):
        return self.princ_values_N[:, 6]
            ...
            for case_name in case_list:
                for direction_name in direction_list:
                    ...
                    if limit_state_name == 'G':
                        outputs_arr = self.get_outputs_GdG( n, m )
                        # global variable!
                        # @todo: this need to be solved differently!
                        #
                        # output_list_GdG is a global variable!
                        # the order in the list must correspond to the returned 
                        # array of get_outputs_GdG
                        outputs_list = outputs_list_GdG

                    elif limit_state_name == 'T':
                        outputs_arr = self.get_outputs_GdT( n, m, alpha )
                        # global variable!
                        # @todo: this need to be solved differently!
                        #
                    ...
                    for output_idx, output_name in enumerate( outputs_list ):
                        # assign: sig_n_Nx
                        #         sig_n_Ny
                        #         sig_n_Mx
                        #         sig_n_My
                        setattr( self, output_name + '_' + case_name + direction_name, outputs_arr[ :, output_idx ] )
...
#                        print 'sort_var_max_GdT', self.sort_var_max_GdT
        ...
        for limit_state_name in limit_state_list:
            for case_name in case_list:
                for direction_name in direction_list:
                    ...
                    self.display_arr = getattr( self, display_list_standard[0] )[:, newaxis]
                    for display_name in display_list_standard[1:]:
                        col = getattr( self, display_name )[:, newaxis]
                        self.display_arr = hstack( [ self.display_arr, col ] )
                    ...
                    arr = self.display_arr[ idx_X, : ][:NRES, :]
    ...
    def _run_evaluation( self ):
        ...
        self._check_input_files_consistency()
