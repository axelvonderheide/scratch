...
def fold_strain( X_pnt, x_pnt ):
    ...
    eps0 = 0.03 * 80
    half_width = 0.05
    if float_abs( x ) < half_width:
        eps_mtx[0, 0] = -eps0 * z
    if float_abs( y ) < half_width:
        if x < 0:
            eps_mtx[1, 1] = -eps0 * z
        else:
            eps_mtx[1, 1] = eps0 * z
...
class OrigSheetModel( IBVModel ):
    ...
    #n_dofs_xy = Int( 8, ps_levels = ( 10, 30, 3 ) )
...
#        return self.fe2d5_quad_serendipity
    ...
    def peval( self ):
        ...
        # Second step - Loading
        #
        # get the displacement at the specified dofs and define 
        # it as boundary conditions.

        bc_corners = [ BCDof( dof = corner_dof, var = 'u', value = U[ corner_dof ] )
                      for corner_dof  in self.corner_dofs ]

        fold_dof = self.fold_dof

        bc_top_load = BCSlice( var = 'f', value = 0.1, dims = [2],
                               slice = self.fe_grid[ fold_dof, fold_dof, -1, :, :, -1 ] )

        self.ts.bcond_list = bc_corners + [ bc_top_load ]
        self.ts.setup()
    ...
    tline_1 = Instance( TLine )
    def _tline_1_default( self ):
        return TLine( min = 0.0, step = 1.0, max = 1.0 )

    tline_2 = Instance( TLine )
    def _tline_2_default( self ):
        return TLine( min = 1.0, step = 1.0, max = 2.0 )
...
#                                                var = 'u', idx = 0, warp = True ),
    ...
    fold_dof = Property
    def _get_fold_dof( self ):
        return ( self.n_dofs_xy / 2 ) - 1

    fets_fold = Property
    @cached_property
    def _get_fets_fold( self ):
        return FETS3D8H( mats_eval = MATS3DElastic( E = 10, initial_strain = fold_strain ) )
    ...
    def _get_fe_grid( self ):
        ### IMPORTANT - to assure that the domain has been constructed first
        self.fe_domain
        return self._fe_grid

    # get the cofs of the corner points
    corner_dofs = Property
    def _get_corner_dofs( self ):
        fe_grid = self.fe_grid
        c1 = fe_grid[ 0, 0, 0 ].dofs[0, 0, :]
        c2 = fe_grid[ -1, 0, 0 ].dofs[-1, 0, :]
        c3 = fe_grid[ 0, -1, 0 ].dofs[0, -1, :]
        c4 = fe_grid[ -1, -1, 0 ].dofs[-1, -1, :]
        return hstack( [c1, c2, c3, c4] )

    ts = Property( depends_on = '+ps_levels' )
    @cached_property
    def _get_ts( self ):
...
#        fold_dof = self.fold_dof - 1
        fold_dof = 7
        print 'XXX', fold_dof
        bc_fix_y = BCSlice( var = 'u', value = 0, dims = [1],
                            slice = fe_grid[fold_dof, fold_dof, 0, :, 0, 0] )
        bc_fix_x = BCSlice( var = 'u', value = 0, dims = [0],
                            slice = fe_grid[fold_dof, fold_dof, 0, 0, 0, 0] )
        bc_fix_z = BCSlice( var = 'u', value = 0.0, dims = [2],
                            slice = fe_grid[fold_dof, fold_dof, 0, :, :, 0] )

        bc_list = [ bc_fix_x, bc_fix_y, bc_fix_z ] # bc_fold1_z ] # , bc_fold2_z, bc_fix_z ]
...
if __name__ == '__main__':
    ...
    sim_model = OrigSheetModel( n_dofs_xy = 10, shape_z = 1 )
