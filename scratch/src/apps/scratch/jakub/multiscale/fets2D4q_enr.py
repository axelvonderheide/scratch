...
from level_set import LevelSet2D, Plane
...
class FETS2D4Q(FETSEval):
    ...
    def get_N_mtx(self,r_pnt):
        ...
        N_std_list = [I_mtx*Nr_geo[0,i] for i in range(0,Nr_geo.shape[1])]
        cx = self._node_coord_map_geo
        Nr = array( [[1/4.*(1 + r_pnt[0]*cx[i,0])*(1 + r_pnt[1]*cx[i,1])*\
                      (self.level_set.get_value(r_pnt)/\
                       abs(self.level_set.get_value(r_pnt))-\
                       self.level_set.get_value(cx[i])/\
                       abs(self.level_set.get_value(cx[i]))) 
                      for i in range(0,4) ]] )
        N_enr_list = [I_mtx*Nr[0,i] for i in range(0,Nr.shape[1])]
        N_mtx_list = N_std_list + N_enr_list
...
#    fets_eval = FETS2D4Q12U(mats_eval = MATS2DScalarDamage(strain_norm = Euclidean())) 
    ls_plane = Plane(B = 1.)
    fets_eval = FETS2D4Q(mats_eval = MATS2DElastic(), level_set = ls_plane) 
