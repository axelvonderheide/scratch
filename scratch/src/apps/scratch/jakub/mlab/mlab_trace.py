...
from apps.scratch.jakub.mlab.mlab_pipe import MVUnstructuredGrid
...
#        return self.sd.rt_bg_domain
    ...
    next_button = ToolbarButton( 'Next',
                                style = 'toolbar' )
    @on_trait_change( 'next_button' )
    def next_ts( self, event = None ):
        self.mvp_mgrid_geo.next_ts()

    prev_button = ToolbarButton( 'Prev',
                                style = 'toolbar' )
    @on_trait_change( 'prev_button' )
    def prev_ts( self, event = None ):
        self.mvp_mgrid_geo.prev_ts()
    ...
    def _get_subfields( self ):
        ...
        return [ RTraceDomainField( var = self.var,
                                ...
                                warp = self.warp,
                                warp_f = self.warp_f,
                                sd = subdomain ) for subdomain in self.sd.serialized_subdomains ]
    ...
    def _get_vtk_data( self ):
        ...
        field_arr = tvtk.DoubleArray( name = '%(name)s_%(step)d' \
                                     % {"name":self.name, "step": self.write_counter} )
        ...
        if self.warp:
            ...
            warp_arr = tvtk.DoubleArray( name = '%(name)s_%(step)d' \
                                        % {"name":self.warp_var, "step": self.write_counter} )
    ...
    def redraw( self ):
        ...
        self.mvp_mgrid_geo.rebuild_pipeline( self.vtk_data, self.write_counter )
    ...
    def add_current_values( self, sctx, U_k, *args, **kw ):
        for sf in self.subfields:
            if sf.skip_domain:
                continue
            sf.add_current_values( sctx, U_k )
...
#        self.writer.file_name = self.var+'%(direction)d%(pos)s_%(step)d.vtk' \
#        % {'direction': (self.idx+1),"pos":self.position, "step": self.write_counter}
        self.writer.file_name = '%(pos)s_%(step)d.vtk' \
        % {"pos":self.position, "step": self.write_counter}
...
#        for sf in self.subfields:
#            sf.idx = self.idx
#            scalars_arr_list.append( sf._get_scalars() )
#        return hstack( scalars_arr_list )
    ...
    view = View( HSplit( VSplit ( VGroup( 'var', 'idx' ),
                                  ...
                                  HGroup( Item( 'prev_button', show_label = False ),
