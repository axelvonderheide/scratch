...
#    field_faces = [[0, 1, 5, 4],[6,7,2,3]]
    ...
    def get_N_mtx(self,r_pnt):
        ...
        N_e_mtx_list = [I_mtx*Nr_geo[0,i]* \
                (signum(self.ls_function(r_pnt[0],r_pnt[1]))-sig_vals[i])\
                 for i in range(0,4)]
        N_e_mtx = hstack(N_e_mtx_list)
        
        N_enr_mtx = hstack((N_mtx[:,:2],N_e_mtx[:,:2],\
                            N_mtx[:,2:4],N_e_mtx[:,2:4],\
                            N_mtx[:,4:6],N_e_mtx[:,4:6],\
                            N_mtx[:,6:],N_e_mtx[:,6:]))
        #N_enr_mtx = hstack((N_mtx,N_mtx))
        #print "coord ", r_pnt
        #print "N_mtx ", N_mtx
    ...
    def get_dNr_psi_mtx(self,r_pnt):
        ...
        dNr_geo = array( [[ 1/4.*cx[i,0]*(1 + r_pnt[1]*cx[i,1])* \
                           (signum(self.ls_function(r_pnt[0],r_pnt[1]))\
                           - sig_vals[i]) for i in range(0,4) ],
...
def example_with_new_domain():
    ...
    fets_eval = FETS2D4Qxfem(mats_eval = MATS2DElastic()) 
...
#                         record_on = 'update'),
                    RTraceDomainField(name = 'Displacement' ,
                                    var = 'u', idx = 1,
                                    record_on = 'update'),
                     RTraceDomainField(name = 'Displacement_ip' ,
                                    var = 'u', idx = 1,
                                    #position = 'int_pnts',
                                    record_on = 'update',
                                    warp = True),
