...
#                            p_N_mtx[:,6:],N_e_mtx[:,6:]))#TODO: generalize that
    ...
    def get_dNr_psi_mtx(self,r_pnt):
        ...
        '''
        p_N_mtx = self.get_parent_N_mtx(r_pnt)
        p_N_row = sum(p_N_mtx,0)[::2]#make 1d array
        p_N_red = vstack((p_N_row,p_N_row))
        p_dNr_mtx = self.get_parent_dNr_mtx(r_pnt)
        
        r, s = array(self.dof_r).T
                
        first = sum(sum((hstack(self.ls_function(r[i],s[i]) *\
                           p_N_red[:,i:i+1] \
                           for i in range(0,self.parent_nnodes) ))))
        
        second = sum(sum(hstack(abs(self.ls_function(r[i],s[i])) *\
                           p_dNr_mtx[:,i:i+1] \
                           for i in range(0,self.parent_nnodes) )))
        
        third = sum(sum(hstack( p_N_red[:,i:i+1]* \
                             abs(self.ls_function(r[i],s[i]))\
                             for i in range(0,self.parent_nnodes) )))
        
        fourth = sum(sum(hstack( p_dNr_mtx[:,i:i+1] * \
                            self.ls_function(r[i],s[i])\
                            for i in range(0,self.parent_nnodes) )))
        
        fifth = second - sign(first)*fourth
        
        p_N_reduced = p_N_red * fifth
        
        first_mtx = p_N_reduced * fifth
        
        second_mtx = p_dNr_mtx * (third - abs (first))
...
def example_with_new_domain():
    ...
    fets_eval = FETS2D4Qxfem(mats_eval = MATS2DElastic(E=2.,nu=0.),
                             mats_eval2 = MATS2DElastic(E=3.,nu=0.)) 
