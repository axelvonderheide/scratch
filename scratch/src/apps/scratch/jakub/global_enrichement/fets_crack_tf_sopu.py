...
class FETSCrackTFSOPU(FETSLSEval):
    ...
    def get_N_mtx(self,r_pnt, node_ls_values, r_ls_value):
        ...
        N_e_s_mtx = hstack([N_mtx_sopu[:,i*4:i*4+2]* \
                    (p_value-sign(node_ls_values[i]))/2.\
                    for i in range(0,4)])#TODO: works just for bilinear now

        N_e_mtx = zeros((4,self.n_e_dofs/3))
        N_e_mtx[0,0:8:2] = N_e_s_mtx[0,0::2]
        N_e_mtx[1,1:8:2] = N_e_s_mtx[1,1::2]
    ...
    def get_dNr_psi_mtx(self,r_pnt, node_ls_values, r_ls_value):
        ...
        dNr_mtx = array( [ dNr_mtx_sopu[:,i]* \
                           (p_value\
                           - sign(node_ls_values[i]))/2. for i in range(0,4) ])#TODO:works just with bilinear
    ...
    def get_B_mtx( self, r_pnt, X_mtx, node_ls_values, r_ls_value ):
        ...
        if dNr_mtx.shape[0] == 1:#1D#TODO:
            ...
            #print 'shapes ',Bx_mtx[6:].shape,  N_mtx_red.shape
