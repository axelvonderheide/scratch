...
from fets_eval_enr import FETSEval_enr
...
class FETS2D4Q_enr(FETSEval_enr):
...
def example_with_new_domain():
    ...
    from dots_eval_enr import  DOTSEval_enr,RTraceDomainField_enr
    from ibvpy.mats.mats2D.mats2D_elastic.mats2D_elastic import MATS2DElastic
    from ibvpy.api import BCDofGroup
    fets_eval = FETS2D4Q_enr(mats_eval = MATS2DElastic()) 
    tseval  = DOTSEval_enr( fets_eval = fets_eval )
    ...
    domain = FEGrid( coord_max = (3.,3.,0.), 
                           ...
                           ls_function = SinLSF(a=1.5,b=2.5))
                           #ls_function = PlaneLSF(a = 0.1,b = 1.,c = -2.55))
                           #ls_function = ElipseLSF(a=2.,b=1., c = 3.3))
    ...
    tstepper = TS( tse = tseval,
         ...
         rtrace_list =  [ 
                     RTraceGraph(name = 'Fi,right over u_right (iteration)' ,
                               ...
                               record_on = 'update'),
                    RTraceDomainField_enr(name = 'Stress' ,
                         var = 'sig_app', idx = 0,
                         record_on = 'update'),
                     RTraceDomainField_enr(name = 'Displacement' ,
                                    var = 'u', idx = 0,
                                    record_on = 'update',
                                    warp = True),
