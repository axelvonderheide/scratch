...
def pdf_phi2 ( x ):
    return sin_pi_distr.pdf( x )

def ppf_phi2 ( x ):
    return sin_pi_distr.ppf( x )
...
real_phi = real_phi_or.reshape( 1, len( real_phi_or ) )
...
pdf_matrix = z_phi * z_le
    ...
    variance_list = []
    for w in U:
            pullouts = cbsf( w, tau, 0, 0.15, 200000, real_le, real_phi, f, 0, 0, 1e15 )
            mean_to_be_integrated = pullouts * pdf_matrix
            #print to_be_integrated
            #tep_one = np.trapz( mean_to_be_integrated, x_tau )
            step_two = np.trapz( mean_to_be_integrated, x_phi )
            ...
            step_two2 = np.trapz( stdev_to_be_integrated, x_phi )
    ...
    mean_plot = plt.plot( U , mean_array / area , 'b', linewidth = 3.0 )
    #lower_plot = plt.plot( U, lower_bound_scatter_band / area, 'b--', linewidth = 3.0 )
    #upper_plot = plt.plot( U, upper_bound_scatter_band / area, 'b--' , linewidth = 3.0 )
...
plt.legend( ( mean_plot ), ( 'Auswertung mit sin2$\phi$', 'Auswertung mit sin(0,5$\pi*\phi)$' ), 'lower right' )
