...
from enthought.traits.api import Enum, Str, Range, Tuple, \
                                 Bool, Trait, Int, Any, Property, Dict
...
class ChacoPlotItem(Item):
    ...
    """
    # Name of the trait that references the maindict data source.
    maindict = Dict()
    ...
    # The orientation of the maindict axis.
    ...
    def __init__(self, maindict, type="line", **traits):
...
class ChacoPlotEditor ( Editor ):
    ...
    def _destroy_plot(self):
        if self._container and self._plot:
            plot = self._plot
            del plot.maindict._data
            del plot.maindict._cached_mask
    ...
    def update_editor(self):
        ...
        try:
            x_values = getattr(self.object, plotitem.maindict)
    ...
    def _update_data(self):
        ...
        if self._plot is None:
            ...
            x_values = getattr(self.object, self.factory.plotitem.maindict)
          #  y_values = getattr(self.object, self.factory.plotitem.value)
            self._plot.maindict.set_data(x_values)
    ...
    #    else:
     #       vtitle = plotitem.value
        ...
        # maindict and value axes.  So we have to search through the underlays for
        # PlotAxis instances whose ranges match the maindict and value ranges.
