    ...
    def _read_exdesign(self):
        ...
        if exists( self.design_file ):
            ...
            for i in range(len(self.names_units_list)):
                ...
                # Here xidx_enum and yidx_enum should be updated
                #self.xidx_enum[ self.names_units_list[i] ] = i
                #self.yidx_enum[ self.names_units_list[i] ] = i
...
#    yidx = Int( 1, auto_set = False, enter_set = True )
    ...
    xidx_enum = Dict({'a':0, 'b':1})  
    yidx_enum = Dict({'a':0, 'b':1}) 
    
    xidx = Str 
    yidx = Str

    def _xidx_changed(self): #, new):
        print 'changed'
        
    def _yidx_changed(self): #, new):
        self._xidx_changed()
...
#            self.plot.plot( (`idx`,`1`), color = colors[idx] )
    ...
    view_traits = View( HSplit( VGroup( Item('open_design',  show_label = False,
                                             style = 'simple' ),
#                                        Item('xidx_enum', style='simple'),
#                                        Item('yidx_enum', style='simple'),
                                        Item( name='xidx', editor=EnumEditor(name='xidx_enum')),
                                        Item( name= 'yidx', editor=EnumEditor(name='yidx_enum')),
