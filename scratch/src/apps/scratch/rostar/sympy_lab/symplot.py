...
def sample(f, *var_args):
    ...
    f is a function of one or 
print X
XX = 1 - exp(-l_f/l_r*(sigma_y/sigma_r)**m) - P
print solve(XX,sigma_y)two variables, such as x**2.
...
def mplot2d(f, var, show=True):
    ...
    for f_i in f:
        ...
        p.title('Graph')
        p.xlabel("$x$", size = "large")
        p.ylabel("$y$", size = "large")
        #p.ylim(1.0,1.9)
        #p.xlim(1.0,15)
        p.grid(color='gray', linestyle='--', linewidth=0.1, alpha = 0.4)
        p.draw()
        if show:
            p.show()
