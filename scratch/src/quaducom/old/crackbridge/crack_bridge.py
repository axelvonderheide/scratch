...
class PullOutWithConstantFriction( RF ):
    ...
    title = Str( 'stress profile in a fiber with a constant friction to stiff matrix ' )
    ...
    # 
    def __call__( self, u, tau, l, E_f, D_f ):
        ...
        #clamped
#        cs = 50
#        u0 = ( ( cs * T + T * l ) ** 2 - ( l * T ) ** 2 ) / ( 2 * E_f * A_f * T )
#        q = q * Heaviside( T * cs - q ) + ( E_f * A_f * ( u - u0 ) / cs + T * cs ) * Heaviside( q - cs * T )

        return q
        # l + q / T - this would be the stress transfer length of a single fiber
...
class StressInFiberWithConstantFriction( PullOutWithConstantFriction ):
    ...
    def __call__( self, x, u, tau, l, E_f, D_f ):
        ...
        xi = x - l
        return ( q - ( T * xi * Heaviside( xi ) ) ) * Heaviside( q - T * xi )

if __name__ == '__main__':

#    e = linspace( 0, 0.0003, 100 )
#    P = PullOutWithConstantFriction()
#    plt.plot( e, P( e, 3.4, 0.0, 70e6, 26e-3 ) )
#    plt.show()
